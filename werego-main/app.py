...
def user_list():
    ...
    if session['user_type'] == 'admin':
        db=Db()
        qry = db.select("SELECT username FROM login WHERE usertype='user'")
...
def station_search():
    if 'user_type' in session and session['user_type'] == 'user':
        ...
        db = Db()
        sql = "select * from admin_charging_station_list where city = %s and charger_type = %s and status ='Active'"
