...
def returnAscii():
    ...
    print(latitude)
    print(longitude)
    owmkey='c4a2fce03b702e04409a330d7ce93b0d'
    API_URL = f"http://api.weatherapi.com/v1/current.json?key=ef67efc7ab6d48cd89540707230204&q={latitude},{longitude}&aqi=no"
    print(API_URL)
    url = f"https://api.openweathermap.org/data/2.5/weather?lat={latitude}&lon={longitude}&appid=c4a2fce03b702e04409a330d7ce93b0d"
    forecast = f"http://api.openweathermap.org/data/2.5/forecast?lat={latitude}&lon={longitude}&appid=c4a2fce03b702e04409a330d7ce93b0d"
    response = requests.get(API_URL)
    response2 = requests.get(url)
    url = f"https://api.opencagedata.com/geocode/v1/json?q={latitude}+{longitude}&key=c600ba9900804ce1b87cb607864319a9"
    responseP = requests.get(url).json()
    print(responseP)
    place_nameP = responseP['results'][0]['formatted'] 
    print('NAMEP--',place_nameP)
    place_na = str(place_nameP).split(',')
    print('NA----',place_na)
    city = place_na[2].split('-')[0]
    print('NA2----',city)
    forecastresponse = requests.get(forecast)
    print('FR----',forecastresponse)
    ...
    loaded_model=pickle.load(open('C:/Users/hp/Documents/FlutterProjects/AirQualityPredic/randomFModel.pkl', 'rb'))
    ...
    with open('C:/Users/hp/Documents/FlutterProjects/AirQualityPredic/randomFModel.pkl', 'rb') as f:
